    ...
    <el-table :data="things" style="width: 100%">
      ...
      <el-table-column label="Operations">
        <template slot-scope="scope">
          ...
          <el-button size="mini" v-on:click="edit(scope.$index)"> Update </el-button>
...
export default {
  ...
  methods: {
    // TODO: CRUD of the database might be a service in the future.
    create: function(formName) { // Create item
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('http://localhost:3000/api/things', this.thingForm)
          .then(response => {
